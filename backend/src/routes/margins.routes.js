...
// GET /api/margins/journey?crop=<name>&quantity=<n>
// Returns the price journey for a crop: retail price + each hop's take
// on the traditional path, plus the AgroMart direct path for comparison.
router.get('/journey', async (req, res, next) => {
  try {
    const { crop, quantity = 1 } = req.query;
    if (!crop) return res.status(400).json({ error: 'crop query param required' });
    const qty = Math.max(1, Number(quantity));
    ...
    const priceRow = await fetchOne(
      `SELECT AVG(retail_price)::numeric(10,2) AS retail_price,
              AVG(wholesale_price)::numeric(10,2) AS wholesale_price,
              MAX(unit) AS unit
       FROM market_prices
       WHERE crop_name = $1
         AND price_date = (SELECT MAX(price_date) FROM market_prices WHERE crop_name = $1)`,
    ...
    const retailPrice = Number(priceRow.retail_price);
    const wholesalePrice = Number(priceRow.wholesale_price);
    const unit = priceRow.unit || 'kg';
    const totalRevenue = retailPrice * qty;
    ...
    // Build the traditional journey in supply-chain order (farmer → dealer → wholesaler → retailer)
    const ROLE_ORDER = ['farmer', 'dealer', 'wholesaler', 'retailer'];
    const traditional = ROLE_ORDER.map((role) => {
      const m = margins.find((x) => x.system === 'traditional' && x.role === role);
      if (!m) return null;
      return {
        role,
        share_pct: Number(m.share_pct),
        amount: +(retailPrice * m.share_pct / 100).toFixed(2),
        cumulative_pct: 0, // filled below
      };
    }).filter(Boolean);

    // cumulative_pct = running total up to this role
    let running = 0;
    traditional.forEach((r) => {
      running += r.share_pct;
      r.cumulative_pct = running;
    });

    // AgroMart: farmer + platform fee
    const AGRO_ORDER = ['farmer', 'platform'];
    const agromart = AGRO_ORDER.map((role) => {
      const m = margins.find((x) => x.system === 'agromart' && x.role === role);
      if (!m) return null;
      return {
        role,
        share_pct: Number(m.share_pct),
