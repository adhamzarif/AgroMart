// HigherMargins.jsx — polished calculator page showing farmer's earning boost with AgroMart.
  ...
  return (
    <div className="bg-gray-50 pb-20">
      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        {!loading && data && (
          <>
            <div className="relative mb-8 overflow-hidden rounded-3xl border border-green-100 bg-gradient-to-br from-green-50 via-emerald-50 to-white p-8 shadow-lg sm:p-10">
              <div className="pointer-events-none absolute -right-16 -top-16 h-64 w-64 rounded-full bg-green-200/40 blur-3xl" />
              <div className="pointer-events-none absolute -bottom-20 -left-10 h-56 w-56 rounded-full bg-emerald-200/40 blur-3xl" />
              <div className="relative">
                <div className="inline-flex items-center gap-2 rounded-full bg-white/70 px-4 py-1 text-xs font-semibold uppercase tracking-wider text-green-800 shadow-sm backdrop-blur">
                  💰 {t('hm_headline')}
                </div>
                <div className="mt-4 text-6xl font-bold text-gray-900 font-display sm:text-7xl">
                  ৳{fmt(data.gain)}
                </div>
                <div className="mt-3 flex flex-wrap items-baseline gap-3">
                  <span className="text-lg text-gray-700">{t('hm_more_per_sale')}</span>
                  <span className="rounded-full bg-green-600 px-3 py-1 text-sm font-bold text-white">
                    +{fmt(data.gain_pct)}% {t('hm_gain_pct')}
                  </span>
                </div>
                <div className="mt-4 text-sm text-gray-500">
                  {fmt(data.quantity)} {data.unit} × ৳{fmt(data.retail_price)} = ৳{fmt(data.total_revenue)} {t('hm_total_retail')}
                </div>
              </div>
            </div>
            ...
            <div className="mb-8 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm sm:p-8">
                ...
                <ResponsiveContainer>
                  <BarChart data={chartData} layout="vertical" margin={{ top: 10, right: 20, left: 10, bottom: 10 }}>
                    ...
                    <YAxis
                      type="category"
                      dataKey="label"
                      tick={{ fontSize: 14, fill: '#111827', fontWeight: 700 }}
                      axisLine={false}
                      tickLine={false}
                      width={140}
                    />
                    <Tooltip
                      cursor={{ fill: '#f8fafc' }}
                      formatter={(v, name) => [`৳${fmt(Number(v))}`, roleLabel(name)]}
                      contentStyle={{
                        borderRadius: 12,
              ...
              <div className="mt-6 flex flex-wrap gap-4 text-xs">
                {['farmer','dealer','wholesaler','retailer','platform'].map((role) => (
                  <span key={role} className="inline-flex items-center gap-2 text-gray-600">
...
function SummaryCard({ title, subtitle, farmerEarn, totalRevenue, fmt, t, tone, highlight }) {
  const bg = tone === 'green'
    ? 'border-green-200 bg-gradient-to-br from-green-50 to-white'
    : 'border-red-100 bg-gradient-to-br from-red-50 to-white';
  const pct = totalRevenue > 0 ? (farmerEarn / totalRevenue * 100).toFixed(0) : 0;
  return (
    <div className={`relative rounded-2xl border p-6 shadow-sm sm:p-7 ${bg} ${highlight ? 'ring-2 ring-green-500 ring-offset-2' : ''}`}>
      {highlight && (
        <span className="absolute -top-3 right-4 rounded-full bg-green-600 px-3 py-1 text-xs font-bold text-white shadow-md">
          ✓ {t('hm_best')}
        </span>
      )}
      ...
      <div className="mt-5">
        <div className="text-xs uppercase tracking-wider text-gray-400">{t('hm_farmer_gets')}</div>
        <div className="mt-1 text-3xl font-bold text-gray-900 font-display sm:text-4xl">
          ৳{fmt(farmerEarn)}
        </div>
        <div className="mt-1 text-xs text-gray-500">
          ({fmt(pct)}% {t('hm_of_total')})
        </div>
      </div>
    </div>
  );
...
function BreakdownList({ title, rows, roleLabel, fmt, total, highlight }) {
  return (
    <div className={`border-r border-gray-100 p-6 last:border-r-0 sm:p-8 ${highlight ? 'bg-green-50/40' : ''}`}>
      <h3 className="mb-5 font-bold text-gray-900">{title}</h3>
        ...
        {rows.map((r) => (
          <div key={r.role}>
            ...
            <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-gray-100">
              <div
                className={r.role === 'farmer' ? 'h-full bg-green-500' : 'h-full bg-gray-300'}
